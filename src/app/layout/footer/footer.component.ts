          ...
          <div>
            ...
            <div class="flex gap-3">
              @for (social of socials; track social.name) {
                <a href="#" class="w-9 h-9 bg-[#2D6F6B] text-white rounded-full flex items-center justify-center hover:bg-[#1E4D4A] transition-colors shadow-sm">
                  <span class="sr-only">{{ social.name }}</span>
                  <div [innerHTML]="social.icon" class="flex items-center justify-center"></div>
                </a>
              }
            </div>
          </div>
