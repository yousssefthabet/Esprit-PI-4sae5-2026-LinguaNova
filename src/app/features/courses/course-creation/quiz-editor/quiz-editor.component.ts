  ...
  template: `
    <div class="space-y-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        <div class="md:col-span-12 lg:col-span-7 flex flex-col gap-2">
        ...
        <div class="md:col-span-6 lg:col-span-3 flex flex-col gap-2">
        ...
        <div class="md:col-span-6 lg:col-span-2 flex flex-col items-center justify-center pt-6 lg:pt-8">
           <label class="flex items-center gap-3 cursor-pointer group">
              <div class="relative w-6 h-6 shrink-0">
                  <input type="checkbox" [(ngModel)]="quiz.isFinalExam" (ngModelChange)="onQuizChange()" class="peer absolute inset-0 opacity-0 cursor-pointer z-10" />
                  <div class="w-6 h-6 rounded-lg border-2 border-gray-200 peer-checked:bg-purple-500 peer-checked:border-purple-500 transition-all"></div>
              ...
              <span class="text-xs font-black text-gray-400 uppercase tracking-widest group-hover:text-purple-500 transition-colors">Final Exam</span>
           </label>
        </div>
      </div>
      ...
      <div class="space-y-6">
        <div class="flex items-center justify-between mb-8 pb-4 border-b border-gray-50">
          ...
          <button (click)="addQuestion()" class="px-5 py-2.5 bg-[#0D9488] text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-teal-100 hover:bg-[#0D5E5B] transition-all active:scale-[0.98]">
             + Add Question
          </button>
        </div>
        ...
        <div class="space-y-6">
          @for (q of quiz.questions; track q.id; let i = $index) {
            <div class="p-8 bg-gray-50/50 rounded-[32px] border border-gray-100 relative group/q hover:bg-white hover:shadow-xl hover:shadow-gray-200/20 transition-all duration-300">
              <button (click)="removeQuestion(i)" class="absolute top-6 right-6 text-gray-300 hover:text-red-500 transition-colors">
                ...
                <div class="space-y-3">
                  ...
                  @for (opt of q.options; track $index) {
                    <div class="flex items-center gap-4 group/opt">
                      ...
                      <button (click)="removeOption(i, $index)" class="opacity-0 group-hover/opt:opacity-100 text-gray-300 hover:text-red-500 transition-all">
                  ...
                  <div class="pt-2 pl-9">
                    <button (click)="addOption(i)" class="text-[10px] font-black text-[#0D9488] uppercase tracking-widest hover:underline">+ New Option</button>
                  </div>
                </div>
              </div>
            </div>
