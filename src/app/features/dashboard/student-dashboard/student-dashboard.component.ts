    ...
    <div class="bg-[#F8FAFC] min-h-screen pb-20 font-sans">
      ...
      <div class="container mx-auto px-4 max-w-[1200px] py-12">
        ...
        <div class="grid lg:grid-cols-12 gap-10">
          ...
          <!-- Upcoming Classes (Left 8 cols) -->
          <div class="lg:col-span-8 space-y-8">
            <div class="flex items-center justify-between mb-2">
               <div>
                  <h2 class="text-2xl font-black text-gray-900 tracking-tight">Your Next Classes</h2>
                  <p class="text-gray-400 text-sm font-medium">Don't miss out on your live sessions</p>
               </div>
               <a routerLink="/live-class" class="text-[#0D9488] font-bold text-sm hover:underline">View All Sessions</a>
            </div>

            <div class="space-y-4">
              @for (cls of upcomingClasses; track cls.id) {
                <div class="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm hover:shadow-md hover:border-teal-100 transition-all group overflow-hidden relative">
                  <!-- Live Indicator -->
                  @if (cls.status === 'live') {
                    <div class="absolute top-0 right-0 px-4 py-1.5 bg-red-500 text-white text-[10px] font-black uppercase tracking-widest rounded-bl-xl shadow-lg animate-pulse">
                       LIVE NOW
                    </div>
                  }
                  ...
                  <div class="flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
                    <div class="flex items-center gap-6 w-full sm:w-auto">
                      ...
                      <div>
                        <h3 class="font-bold text-gray-900 group-hover:text-[#0D9488] transition-colors text-xl mb-1 tracking-tight">
                          {{ cls.courseName }}
                        </h3>
                        <div class="flex items-center gap-3 text-sm font-medium text-gray-500 italic">
                          <span>Instructor: {{ cls.instructorName }}</span>
                          <span class="w-1 h-1 rounded-full bg-gray-300"></span>
                          <span class="text-[#0D9488]">{{ cls.startTime | date:'HH:mm' }} - {{ cls.endTime | date:'HH:mm' }}</span>
                        </div>
                      </div>
                    </div>

                    <button 
                      [class]="cls.status === 'live' 
                        ? 'px-8 py-3 bg-[#0D9488] text-white font-bold rounded-xl shadow-lg shadow-teal-100' 
                        : 'px-8 py-3 bg-white border border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50'"
                      class="w-full sm:w-auto transition-all active:scale-95"
                    >
                      {{ cls.status === 'live' ? 'Join Classroom' : 'Session Info' }}
                    </button>
                  </div>
                </div>
              }
